...
<div x-data="{ open: false }" class="relative">
    <button @click="open = !open" class="relative p-2 rounded-lg text-slate-400 hover:bg-slate-800 transition">
        ...
        <div class="flex items-center justify-between px-4 py-3 border-b border-slate-100">
            ...
            @if($unreadCount > 0)
                <form method="POST" action="{{ route('notifications.markAllRead') }}">
                    @csrf
                    <button type="submit" class="text-xs text-teal-600 hover:text-teal-800">Всі прочитані</button>
                </form>
            @endif
        </div>
