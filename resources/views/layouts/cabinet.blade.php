...
<head>
    ...
    <title>@yield('title', 'Кабінет майстра') — MasterCall</title>
...
<header class="sticky top-0 z-40 bg-slate-900 text-white shadow-sm">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between h-16" x-data="{ mobileOpen: false }">
            <a href="{{ route('home') }}" class="flex items-center gap-2 font-bold text-xl text-teal-400" style="font-family: 'Space Grotesk', sans-serif;">
            ...
            <nav class="hidden md:flex items-center gap-1 text-sm font-medium">
                <a href="{{ route('cabinet.orders.index') }}" class="px-3 py-2 rounded-lg {{ request()->routeIs('cabinet.orders*') ? 'bg-slate-700 text-white' : 'text-slate-300 hover:bg-slate-800' }} transition">Замовлення</a>
                <a href="{{ route('cabinet.services.index') }}" class="px-3 py-2 rounded-lg {{ request()->routeIs('cabinet.services*') ? 'bg-slate-700 text-white' : 'text-slate-300 hover:bg-slate-800' }} transition">Послуги</a>
                <a href="{{ route('cabinet.profile.edit') }}" class="px-3 py-2 rounded-lg {{ request()->routeIs('cabinet.profile*') ? 'bg-slate-700 text-white' : 'text-slate-300 hover:bg-slate-800' }} transition">Профіль</a>
            </nav>

            <div class="hidden md:flex items-center gap-3">
                @include('components.notification-bell-dark')
                <span class="text-sm text-slate-300">{{ auth()->user()->name }}</span>
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit" class="text-sm text-slate-400 hover:text-red-400 transition">Вийти</button>
                </form>
            </div>

            <button @click="mobileOpen = !mobileOpen" class="md:hidden p-2 rounded-lg text-slate-300 hover:bg-slate-800">
        ...
        <div x-show="mobileOpen" x-transition class="md:hidden pb-4 border-t border-slate-700 mt-1">
            <nav class="flex flex-col gap-2 pt-3 text-sm">
                <a href="{{ route('cabinet.orders.index') }}" class="text-slate-300 hover:text-white py-1">Замовлення</a>
                <a href="{{ route('cabinet.services.index') }}" class="text-slate-300 hover:text-white py-1">Послуги</a>
                <a href="{{ route('cabinet.profile.edit') }}" class="text-slate-300 hover:text-white py-1">Профіль</a>
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit" class="text-left text-red-400 py-1">Вийти</button>
                </form>
            </nav>
        </div>
    </div>
